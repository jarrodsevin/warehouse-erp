  ...
  const handleEmailPDF = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/email', {
        ...
        body: JSON.stringify({
          to: emailAddress,
          subject: `Subcategory Profitability Analysis - ${new Date().toLocaleDateString()}`,
          html: `
            <h2>Subcategory Profitability Analysis</h2>
            <p>Please find attached your Subcategory Profitability Analysis report.</p>
            <h3>Summary:</h3>
            <ul>
              <li><strong>Total Subcategories:</strong> ${subcategoryMetrics.length}</li>
          ...
          attachments: [
            {
              filename: `subcategory-analysis-${new Date().toISOString().split('T')[0]}.pdf`,
              content: pdfBase64,
            },
          ],
        }),
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-orange-600">{subMetric.subcategory.name}</h3>
                    <p className="text-sm text-gray-500 mt-1">{subMetric.productCount} products</p>
                  </div>
                </div>
                <div className="flex gap-8">
                  <div className="text-right">
                    <p className="text-sm text-gray-500">Avg Margin</p>
                    <p className="text-2xl font-bold text-yellow-600">{subMetric.avgMargin.toFixed(2)}%</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-500">Avg Markup</p>
                    <p className="text-2xl font-bold text-purple-600">{subMetric.avgMarkup.toFixed(2)}%</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-500">Avg Profit/Unit</p>
                    <p className="text-2xl font-bold text-green-600">${subMetric.avgProfitPerUnit.toFixed(2)}</p>
                  </div>
